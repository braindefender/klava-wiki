...
на которых расположены `A S D F` и `J K L ;`.
...
В слое с модификаторами есть левые и правые модификаторы (LAlt и RAlt).
Звучит логично ведь? Левые слева, правые — справа.

Однако, RAlt в большинстве систем, это не просто RAlt, а так называемый AltGr.
Фактически, это Ctrl+Alt. Зачем это было сделано?
Хрен его знает, но так устроены клавиатуры и их обработка в системе.
...
- зажать `RShift` и `RCtrl` указательным и средним пальцами правой руки. Именно **зажать**.
...
чтобы нажать `T`. И так придётся делать для всех комбинаций клавиш. Звучит не слишком удобно.
...
Поэтому Home-Row Mods в классической реализации мне не подходит. К тому же, я не всегда держу руки на клавиатуре.
...
Можно выставить таймер сброса `OSS` в какое-то очень большое число
...
Случайных нажатий при обычной печати не произойдёт, потому что `OSM` находится в слое.
...
Причём, можно настроить `OSM` так, чтобы он приводил модификатор в состояние зажатия.
Это называется One-Shot Sticky Modifier или `OSSM` (или non-lazy OSM режим в ZMK)
